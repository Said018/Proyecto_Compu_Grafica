...
int FrameIndex = 8;			//introducir número en caso de tener Key guardados
bool play = false;
int playIndex = 0;
...
void LoadTextures()
{
	...
	t_unam = generateTextures("Texturas/color_121212.hdr", 0, true);
...
int main() {
	...
	Model helice("resources/objects/Dron/Helice.fbx");
	Model dron("resources/objects/Dron/Dron.fbx");
	/*
	...
	Model casaDeBruja("resources/objects/CasaBruja/Casa.obj");
	Model sillaEscritorio("resources/objects/Silla/Silla.obj");*/
	//Model aquamanCompleto("resources/objects/Aquaman/Aquaman.obj");
/*	Model cuerpoAquaman("resources/objects/Aquaman/Cuerpo.obj");
	Model cabezaAquaman("resources/objects/Aquaman/Cabeza.obj");
	Model braDerAquaman("resources/objects/Aquaman/BrazoDerecho.obj");
	Model braIzqAquaman("resources/objects/Aquaman/BrazoIzquierdo.obj");
	Model pieDerAquaman("resources/objects/Aquaman/PiernaDerecha.obj");
	Model pieIzqAquaman("resources/objects/Aquaman/PiernaIzquierda.obj");
	Model botaDerAquaman("resources/objects/Aquaman/BotaDerecha.obj");
	Model botaIzqAquaman("resources/objects/Aquaman/BotaIzquierda.obj");

	ModelAnim animacionPersonaje("resources/objects/Personaje1/Arm.dae");
	animacionPersonaje.initShaders(animShader.ID);
	
	ModelAnim animacionNueva("resources/objects/Animacion/animacion.dae");
	animacionNueva.initShaders(animShader.ID);

	ModelAnim animacionSentado("resources/objects/Sentado/sentado.dae");
	animacionSentado.initShaders(animShader.ID);
*/
	...
	glm::mat4 tmpPieDerAquaman = glm::mat4(1.0f);
	glm::mat4 tmpPieIzqAquaman = glm::mat4(1.0f);
*/
	glm::mat4 tmpDron = glm::mat4(1.0f);
...
/*
		...
		modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(150.0f, 0.0f, 0.3f)); // translate it down so it's at the center of the scene
		modelOp = glm::scale(modelOp, glm::vec3(0.1f));
		animShader.setMat4("model", modelOp);
		animacionNueva.Draw(animShader);

		// -------------------------------------------------------------------------------------------------------------------------
		// Animacion Nueva
		// -------------------------------------------------------------------------------------------------------------------------
		
		modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(100.0f, 0.0f, 0.3f)); // translate it down so it's at the center of the scene
		modelOp = glm::scale(modelOp, glm::vec3(0.1f));
		animShader.setMat4("model", modelOp);
		animacionSentado.Draw(animShader);
*/
		...
		*/
		...
		staticShader.setMat4("view", viewOp);
		/* Aquaman completo
		modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(2.0, 10.0f, 0.0f));
		staticShader.setMat4("model", modelOp);
		aquamanCompleto.Draw(staticShader);

		modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(posX, posY, posZ));
		tmpCuerpoAquaman = modelOp = glm::rotate(modelOp, glm::radians(giroMonito), glm::vec3(0.0f, 1.0f, 0.0f));
		staticShader.setMat4("model", modelOp);
		cuerpoAquaman.Draw(staticShader);
		...
		modelOp = glm::rotate(modelOp, glm::radians(giroBraDer), glm::vec3(1.0f, 0.0f, 0.0f));
		staticShader.setMat4("model", modelOp);
		braDerAquaman.Draw(staticShader);

		modelOp = glm::translate(tmpCuerpoAquaman, glm::vec3(0.75f, 1.5f, 0.0f));
		modelOp = glm::rotate(modelOp, glm::radians(giroBraDer), glm::vec3(-1.0f, 0.0f, 0.0f));
		staticShader.setMat4("model", modelOp);
		braIzqAquaman.Draw(staticShader);

		modelOp = glm::translate(tmpCuerpoAquaman, glm::vec3(-0.5f, 0.0f, 0.1f));
		tmpPieDerAquaman = modelOp = glm::rotate(modelOp, glm::radians(giroBraDer), glm::vec3(-1.0f, 0.0f, 0.0f));
		staticShader.setMat4("model", modelOp);
		pieDerAquaman.Draw(staticShader);
		
		modelOp = glm::translate(tmpCuerpoAquaman, glm::vec3(0.5f, 0.0f, 0.1f));
		tmpPieIzqAquaman = modelOp = glm::rotate(modelOp, glm::radians(giroBraDer), glm::vec3(1.0f, 0.0f, 0.0f));
		staticShader.setMat4("model", modelOp);
		pieIzqAquaman.Draw(staticShader);
		
		modelOp = glm::translate(tmpPieDerAquaman, glm::vec3(0.0f, -0.9f, -0.1f));
		staticShader.setMat4("model", modelOp);
		botaDerAquaman.Draw(staticShader);
		
		modelOp = glm::translate(tmpPieIzqAquaman, glm::vec3(0.0f, -0.9f, -0.1f));
		staticShader.setMat4("model", modelOp);
		botaIzqAquaman.Draw(staticShader);
*/
		/*
		modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(-150.0f, 0.0f, -10.0f));
		modelOp = glm::scale(modelOp, glm::vec3(4.0f));
		staticShader.setMat4("model", modelOp);
		casaDeBruja.Draw(staticShader);
...
*/
		...
		modelOp = glm::translate(tmpDron, glm::vec3(3.0f, 1.12f, -0.67f));
		modelOp = glm::scale(modelOp, glm::vec3(1.0f));
		modelOp = glm::rotate(modelOp, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		modelOp = glm::rotate(modelOp, glm::radians(giroMonito), glm::vec3(0.0f, 0.0f, 1.0f));
		staticShader.setMat4("model", modelOp);
		helice.Draw(staticShader);
		
		modelOp = glm::translate(tmpDron, glm::vec3(2.97f, 1.12f, 7.97f));
		modelOp = glm::scale(modelOp, glm::vec3(1.0f));
		modelOp = glm::rotate(modelOp, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		modelOp = glm::rotate(modelOp, glm::radians(giroMonito), glm::vec3(0.0f, 0.0f, 1.0f));
		staticShader.setMat4("model", modelOp);
		...
		modelOp = glm::translate(tmpDron, glm::vec3(-3.24f, 1.12f, 7.92f));
		modelOp = glm::scale(modelOp, glm::vec3(1.0f));
		modelOp = glm::rotate(modelOp, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		modelOp = glm::rotate(modelOp, glm::radians(giroMonito), glm::vec3(0.0f, 0.0f, 1.0f));
		staticShader.setMat4("model", modelOp);
		...
		modelOp = glm::translate(tmpDron, glm::vec3(-3.2f, 1.12f, -0.69f));
		modelOp = glm::scale(modelOp, glm::vec3(1.0f));
		modelOp = glm::rotate(modelOp, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		modelOp = glm::rotate(modelOp, glm::radians(giroMonito), glm::vec3(0.0f, 0.0f, 1.0f));
		staticShader.setMat4("model", modelOp);
		...
		modelOp = glm::scale(modelOp, glm::vec3(1.0f));
		modelOp = glm::rotate(modelOp, glm::radians(135.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		staticShader.setMat4("model", modelOp);
